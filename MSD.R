...
labs(x = "Time",y="MSD") + ggtitle(paste("MSD (All particles) - ",prefix,sep=""))
...
b = ggplot(time_avg_MSD, aes(x=tau,y=time_avg)) + geom_errorbar(aes(ymin=time_avg-time_sd, ymax=time_avg+time_sd, color="grey"), width=.05) + geom_line() + theme_bw(base_size = 16) + scale_color_discrete(guide=FALSE) + labs(x="Time (s)",y="MSD") + ggtitle(paste("MSD (Ensemble average) - ",prefix,sep=""))
...
c = ggplot(time_avg_MSD, aes(x=tau,y=time_avg)) + geom_line() + theme_bw(base_size = 16) + scale_color_discrete(guide=FALSE) + labs(x="Time (s)",y="MSD") + ggtitle(paste("MSD (Ensemble average) - ",prefix,sep=""))
...
d = ggplot(time_avg_MSD, aes(x=tau)) + geom_line(aes(y=time_avg, colour="blue")) + theme_bw(base_size = 16) + scale_color_discrete(guide=FALSE) + labs(x="Time (s)",y="MSD") + ggtitle(paste("Ensemble average MSD vs Number of tracks per frame - ",prefix,sep="")) + geom_line(aes(y = n_per_frame/25, colour = "red")) + scale_y_continuous(sec.axis = sec_axis(~., name = "Number of contributing trajectories (x25)"))
