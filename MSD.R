...
single = ggplot(msd_single, aes(x=as.numeric(factor(FRAME))*frame_int,y=VALUE,group=TRACK,color=factor(TRACK))) + geom_line() + theme_bw(base_size = 18) + scale_color_discrete(guide=FALSE) + labs(x = "Time",y="MSD tau") + ggtitle(paste("MSD tau (All particles) - ",prefix,sep=""))
...
both = ggplot(both_MSD) + geom_line(aes(x=TAU*frame_int,y=value,group=variable,color=variable),size=0.7) + theme_bw(base_size = 18) + labs(x="Time (s)",y="MSD") + scale_colour_manual(values=c("firebrick1","dodgerblue"))
...
time_ens = ggplot(MELT_tau, aes(x=TAU*frame_int,y=msd_tau)) + geom_line() + theme_bw(base_size = 18) + scale_color_discrete(guide=FALSE) + labs(x="Time (s)",y="MSD tau") + ggtitle(paste("MSD tau (Time-ensemble average) - ",prefix,sep=""))
time_ens$theme$plot.margin = unit(c(0.5,1,0.5,0.5),"cm")
